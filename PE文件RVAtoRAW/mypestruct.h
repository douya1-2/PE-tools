...
typedef struct _MY_IMAGE_OPTIONAL_HEADER32 {
    ...
    uint32_t AddressOfEntryPoint;     // 入口点RVA（内存中的偏移量）
    uint32_t BaseOfCode;              // 代码段的起始RVA（内存中的偏移）
    uint32_t BaseOfData;              // 数据段的起始RVA（内存中的偏移）
    uint32_t ImageBase;               // 镜像的首地址（32位）（内存中文件基地址）
...
typedef struct _MY_IMAGE_SECTION_HEADER {
    ...
    union {
        ...
        uint32_t VirtualSize;  //内存中节区所占内存大小
    } Misc;
    uint32_t VirtualAddress;       // 节的RVA(内存中节区的起始位置的偏移量)
    uint32_t SizeOfRawData;        // 节的大小（文件中）
    uint32_t PointerToRawData;     // 节的文件偏移（磁盘文件中节区的起始位置）
